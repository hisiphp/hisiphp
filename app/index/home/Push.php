...
class Push extends Home
{
    protected function _initialize() {
        ...
        $this->app_id           = $this->request->param('app_id/d');
        $this->app_name         = strtolower($this->request->param('app_name/s'));
        $this->app_secret_key   = $this->request->param('secret_key/s');
        $this->app_identifier   = strtolower($this->request->param('app_identifier/s'));

        if (empty($this->app_id) || 
            empty($this->sign) || 
            empty($this->token) || 
            empty($this->version) || 
            empty($this->app_identifier) || 
            empty($this->app_name) || 
            empty($this->app_secret_key)) {
    ...
    public function module()
    {
        ...
        // 应用目录
        $appPath = $unzipPath.DS.'upload'.DS.'app'.DS.$this->app_name.DS;
        ...
        if (is_file($appPath.'route.php')) {
            copy($appPath.'route.php', APP_PATH.$this->app_name.'Route.php');
        }

        $info = include_once $appPath.'info.php';

        // 复制app目录
        if (!is_dir($unzipPath.DS.'upload'.DS.'app')) {
        ...
        Dir::copyDir($appPath, './app'.DS.$this->app_name);
        ...
        $sIcon = $unzipPath.DS.'upload'.DS.'app'.DS.$this->app_name.DS.$this->app_name.'.png';
        if (is_file($sIcon)) {
            copy($sIcon, ROOT_PATH.'static'.DS.'app_icon'.DS.$this->app_name.'.png');
            $icon = ROOT_DIR.'static/app_icon/'.$this->app_name.'.png';
        }

        // 复制static目录
        if (is_dir($unzipPath.DS.'upload'.DS.'static')) {
            Dir::copyDir($unzipPath.DS.'upload'.DS.'static', '.'.ROOT_DIR.'static');
        }

        // 复制theme目录
        if (is_dir($unzipPath.DS.'upload'.DS.'theme')) {
            Dir::copyDir($unzipPath.DS.'upload'.DS.'theme', '.'.ROOT_DIR.'theme');
        }
    ...
    public function plugins()
    {
        ...
        // 复制到插件目录
        Dir::copyDir($appPath, '.'.ROOT_DIR.'plugins/'.$this->app_name);
    ...
    public function theme()
    {
        ...
        $appPath = $unzipPath.DS.'upload'.DS.$this->app_name.DS;
        ...
        // 复制到插件目录
        Dir::copyDir($appPath, '.'.ROOT_DIR.'theme/'.$module.'/'.$this->app_name);
