...
class AdminMember extends Model
{
    ...
    public function register($data = [])
    {
        ...
        $level_id = model('AdminMemberLevel')->where('default',1)->value('id');
    ...
    public function login($account = '', $password = '', $remember = false, $field = 'nick,username,mobile,email,avatar', $token = true)
    {
        ...
        $member = self::where($map)->field('id,'.$field.',level_id,password')->find();
        if (!$member) {
    ...
    public function autoLogin($data = [], $oauth = false)
    {
        if ($oauth) {
            ...
            $map['id'] = $data['id'];
            $map['status'] = 1;
            $data = $this->where($map)->field('id,level_id,nick,username,mobile,email,avatar')->find();
            if (!$data) {
                $this->error = '用户不存在或被禁用！';
                return false;
            }
